...
/**
 * Context captured from a partially-completed multipart upload.
 * Enables resume-on-retry for large file uploads.
 */
export interface PartialUploadContext {
  /** Multipart upload identifier (S3/R2 UploadId) */
  uploadId: string;
  /** Object key / storage path */
  key: string;
  /** Parts that were successfully uploaded before the failure */
  completedParts: Array<{ partNumber: number; etag: string }>;
  /** Provider name that originated the partial upload */
  provider: string;
}

/**
 * Thrown when a multipart upload partially completes before failing.
 * Carries the upload context so `withRetry` can resume instead of restarting.
 */
export class PartialUploadError extends MediaError {
  public readonly uploadContext: PartialUploadContext;
  ...
  constructor(
    ...
    super(message, MediaErrorCode.UPLOAD_FAILED, provider, originalError, {
      uploadId: uploadContext.uploadId,
      key: uploadContext.key,
      completedParts: uploadContext.completedParts.length,
    });
